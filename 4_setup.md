...
Vulkanでは、あくまでデータはどこかのメモリのアドレスから始まるデジタルの値なので、GPUがデータの種類を把握するには何らかの手助けの情報が要ります。
その情報こそが``sType``になります。
後から使う上で送られてきたデータをどのように扱えば良いか判断するための情報をGPUに伝えなければならず、
それを``sType``を通じて行うことになっています。

![Vulkanのデータを送るイメージ](4/struct.png "Vulkanのデータを送るイメージ")

``VkInstanceCreateInfo``などの型の情報があれば、それを利用してどのようなデータなのか周知させることができるように思いますが、
それは、C++言語の話しで、GPUはあくまでも教えられたアドレスを指示された通りに使うだけしかできません。
Vulkanのデータでは、いちいち``sType``にデータの種類を「正確」に記録する必要があります。
